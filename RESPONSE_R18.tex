...
\usepackage{hyperref}
\makeatletter
\gdef\copyright@text{Revision R18 --- referee response}
\makeatother
...
\begin{document}
\begin{frontmatter}
\title{Response to the Referee on ``Neural Bellman Operators\textnormal{}}
...
\end{aug}
\begin{abstract}
This response addresses the latest report on R16 and identifies the exact R18 changes, numerical evidence, claim boundaries, and preserved historical material. The original neural-solver objective and accuracy target remain unchanged.
\end{abstract}
\end{frontmatter}
\section{Overview}
We thank the referee for distinguishing verification validity from successful numerical solution. This revision responds through an executed certificate-aligned objective, a theorem linking that objective to policy loss, paired complete-domain experiments, unrestricted classical candidates with method-specific witnesses, a nonlinear full-state control application with an explicit accuracy--work bound, and budget-feasible wealth compensation. The original title, economic model, control opportunities, and $.01$ accuracy target are retained. Historical mathematical and numerical content is preserved.

The final original-economy neural certificates improve substantially but do not reach $.01$. We do not substitute a separate model, a non-neural library, or an arithmetic regression test for that requirement. The following responses distinguish implemented changes, proved statements, and numerical conclusions that the evidence does not establish.

\section{Scientific findings}

\subsection{R16-F0 --- Review-target integrity}
\noindent\emph{Preserved and extended.}

The new canonical review object is ECTA\_R18.tex/PDF, SUPP\_R18.tex/PDF, and RESPONSE\_R18.tex/PDF, with R18\_REVIEW.md as the entry point. The latest report is pinned at 82af00bc296da59e8a3a28ef1bf86c1d6fca2367 and its reviewed manuscript at 2932b74dad6d8d9efce5a114d5098a99ea17ab1f. R18 builds on the already executed R17 science at 982326994c8550db4e039d915c225a9ace8e3044. The new checks verify all 589 entries of the immutable scientific manifest. Historical sources, results, and PDFs are not overwritten. Source, execution, and publication identities are distinguished in the publication manifest.

\subsection{R16-F1 --- Original nonlinear neural accuracy}
\noindent\emph{Substantive certificate progress; the unchanged target is not attained.}
...
\subsection{R16-F2 --- Anti-alignment of the sampled objective}
\noindent\emph{Addressed by an executed certificate-aligned objective and quantitative audit.}
...
\subsection{R16-F3 --- Boundary architecture and finite-budget ablation}
\noindent\emph{Requested matched experiment supplied; no uniform finite-budget advantage claimed.}
...
\subsection{R16-F4 --- Policy-iteration theorem versus implemented algorithm}
\noindent\emph{The methodological center now analyzes the executed algorithm.}

The stationary finite-MDP recurrence is retained, with full proof, under the explicitly auxiliary heading “An auxiliary finite-Bellman policy-iteration bound.” It is not used to claim that the historical actor/critic implemented certified policy iteration. The primary method is now defined by its actual proposal graph, complete certificate, and strict acceptance rule, and the central objective theorem applies to that graph. We additionally prove a finite-horizon defect allocation, with total error $2q^Nb+\sum_n q^n(2\epsilon_n+\eta_n)$, and exact-rational regression tests. Time-local defects scaled by $C_\rho(h)$ accumulate as $C_\rho(T)$, without importing a stationary squared resolvent. This is not represented as an executed continuous-time policy-iteration transfer theorem: the primary neural and classical policies are checked directly in continuous time.

\subsection{R16-F5 --- Two-mode inventory example and dimensional significance}
\noindent\emph{A nonlinear full-state control problem and explicit uniform accuracy theorem are added.}
...
\subsection{R16-F6 --- Restricted baselines and a poor common critic}
\noindent\emph{Both design confounds are removed; sharp matched accuracy remains unestablished.}
...
\subsection{R16-F7 --- Accuracy versus work and approximation floors}
\noindent\emph{Actual certificate/work sequences and separate obstruction/accuracy results supplied.}
...
\subsection{R16-F8 --- Neural identity versus the specialized sharp solver}
\noindent\emph{The neural-solver objective and title are preserved; numerical objects remain separate.}
...
\subsection{R16-F9 --- MPFR and shared mathematical premises}
\noindent\emph{The independent-arithmetic scope is preserved.}
...
\subsection{R16-F13 --- Economic model scope}
\noindent\emph{A new nonlinear multi-product application is added with its structure stated.}
...
\subsection{R16-T1 and T2 --- Exact deterioration statement and loss mismatch}
The historical text now says all ten certificates worsen, and the new table/diagnostic records that sampled MSE falls for nine seeds. Exact ratios and all endpoint losses remain in diagnostics/r16\_loss\_mismatch.json.

\subsection{R16-T3 and T4 --- Refinement floor and multi-seed ablation}
...
\subsection{R16-T5 and T6 --- Baseline restrictions and comparison validity}
...
\subsection{R16-T7 and T8 --- Validated Riccati baseline and total loss}
...
\subsection{R16-T9 --- Stationary amplification constants}
...
\subsection{R16-T10 --- The price-library floor}
...
\subsection{R16-T11 and T12 --- Shared arithmetic premises and machine contract}
...
\subsection{R16-T13 --- Control differences}
Full-domain final ranges and uniform changes from initialization are retained for every accessible seed. The new range audit also covers all six unrestricted classical policies and all twenty fresh-library actors; the supplement provides a complete table. Classical controls are original-action-clipped exact-real bilinear state interpolants, without a neural wealth-distance multiplier. The library ranges hold for all selected prices in $[.5,8]$ and every pre-stopping time. These are rigorous ranges, not sampled confidence intervals or control-optimality statements.

\subsection{R16-T14 --- One executable core algorithm}
...
\section{Review gates and reproduction}
The original accurate neural-solution gate remains unmet at $.01$, and a sharp original-economy matched-accuracy frontier is not established. The operational certificate-aware training gate is addressed by the actual complete-cover algorithm, independently retained raw proposals, and the new objective implication. The previous stationary policy-iteration claim is now explicitly auxiliary; no unexecuted continuous-time transfer is asserted. The nonlinear high-dimensional application supplies its own state-uniform accuracy--work theorem under stated strong convexity, and the wealth adjustment supplies sufficient budget-feasible compensation for the separately identified library. The paper retains a neural-solver identity without borrowing the comparator's accuracy.

The R18 entry point checks all 589 immutable scientific files, audits 60 checkpoint objectives and 61,440 cells, replays two full-cylinder MPFR certificates, rechecks all 18 final nonlinear neural query plans, and runs 450 exact-rational arithmetic checks and 80 exact-rational finite-MDP cases. Pointwise derivative diagnostics are not global proofs. Complete signed tables, control ranges, nonlinear budget curves, implementation semantics, and commands are in the supplement.

The latest review report, preceding manuscripts, networks, source code, and raw numerical results remain unchanged. The publication manifest distinguishes the review input, scientific input, new source commit, execution records, and published artifacts. The new revision branch and its referee copy point to the same publication commit. The machine-readable status map deliberately does not equate build success with resolution of every scientific objection.
\end{document}
