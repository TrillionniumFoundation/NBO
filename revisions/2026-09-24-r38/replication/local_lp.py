...
def quantize_lower(f,bits=40):
    den=1<<bits
    def down(x):return F(x.numerator*den//x.denominator,den)
    g=k.PW(f.xs[:],[(down(a),down(b)) for a,b in f.ab],[down(y) for y in f.ys]).norm()
    delta=k.linear_comb([f,g],[O,-O]);lo,hi=delta.extent()
    assert 0<=lo<=hi<=F(2,den)
    return g

def integral_interval(f,bits=64):
    den=1<<bits;lo=Z;hi=Z
    for (a,b),l,r in zip(f.ab,f.xs,f.xs[1:]):
        v=a*(r*r-l*l)/2+b*(r-l)
        d=v.numerator*den//v.denominator
        lo+=F(d,den);hi+=F(d+int(F(d,den)!=v),den)
    return lo,hi
...
def adaptive(L,U,raw,eps,tol):
    ...
    for t in reversed(range(T)):
        qs=k.q_functions(U[t+1]);zs=[k.linear_comb([q,k.intervention(raw[t],a)],[O,O]) for a,q in enumerate(k.q_functions(A[t+1],False))]
        b=k.linear_comb([L[t]],[O],b=-eps)
        lower,cert=solve_cell_lp(qs,zs,b,tol/2);floored=quantize_lower(lower);A[t],_=k.envelope([floored,k.affine()])
    ...
    for eps in (F(1,100),F(1,20)):
        ...
        for witness in ('constructed','exact'):
            ...
            for tol in (F(1,100),F(1,200),F(1,400)):
                ...
                stage_error=tol/2+F(2,1<<40);err=stage_error*sum(k.BETA**j for j in range(4));lo,ahi=integral_interval(A[0]);hi=ahi+err;grid,gridhi=integral_interval(G[0])
                assert grid<=hi
                ...
                row={'epsilon':str(eps),'witness':witness,'tolerance':str(tol),'support_floor':'0','grid_lower':str(grid),
                     'continuous_local_lower':str(lo),'continuous_local_upper':str(hi),'certified_grid_error_lower':str(max(Z,lo-gridhi)),
                     'certified_grid_error_upper':str(hi-grid),'whole_cells_verified':count,'seconds':time.perf_counter()-tic,
                     'grid_seconds':gridtime,'quantization_bits':40,'per_stage_error':str(stage_error),'integral_rounding_width':str(ahi-lo),'max_pieces':max(len(v.ab) for v in A),'proof_file':'local_lp/'+filename,'proof_sha256':h}
