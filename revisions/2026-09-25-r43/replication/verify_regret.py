...
def verify(path):
    ...
    assert o['schema']=='nbo-r43-regret-v2'
    ...
    rl=[Q(0)]*len(boxes); ru=[Q(0)]*len(boxes); constant=Q(0)
    for i in range(n): rl[names['S',0,i]]=ru[names['S',0,i]]=-initial[i]
    for yy,(row,rhs) in zip(y,equal):
        constant+=floorq(yy*rhs)
        for j,v in row.items():
            term=-yy*v; rl[j]+=floorq(term); ru[j]-=floorq(-term)
    for zz,(row,rhs) in zip(multipliers,less):
        constant+=floorq(-zz*rhs)
        for j,v in row.items():
            term=zz*v; rl[j]+=floorq(term); ru[j]-=floorq(-term)
    lb=inner(initial,refcost[0])+constant
    for cl,cu,(lo,hi) in zip(rl,ru,boxes): lb+=floorq(min(cl*lo,cl*hi,cu*lo,cu*hi))
    lb=max(Q(0),lb)
