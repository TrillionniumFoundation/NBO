...
import numpy, scipy
from regret import model, solve, write_proof, reference, enc, floor_grid
...
def theoretical(d):
    ...
    for t in range(T-1,-1,-1):
        if t==T-1: e=F(0)
            ...
            e=b*eps**2*c/4
        e=-floor_grid(-e)
        ed=-floor_grid(-(e+b*ed)); es=-floor_grid(-((hi-lo)*e+b*es)); maxed=max(maxed,ed)
...
def main():
    protocol=json.loads((ROOT/'PROTOCOL_AMENDED.json').read_text())
    ...
    save(ROOT/'results'/'environment.json',dict(python=sys.version,numpy=numpy.__version__,scipy=scipy.__version__,platform=platform.platform(),threads={k:os.environ.get(k) for k in ['OPENBLAS_NUM_THREADS','OMP_NUM_THREADS']},protocol_sha256=hashlib.sha256((ROOT/'PROTOCOL_AMENDED.json').read_bytes()).hexdigest(),source_sha256=protocol['source_sha256']))
    rows=[]
