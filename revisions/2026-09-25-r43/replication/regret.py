...
def residual_bound(lp,y,lam):
    ...
    lower=[ZERO]*len(lp.keys); upper=[ZERO]*len(lp.keys)
    for j,c in lp.obj.items(): lower[j]=upper[j]=c
    constant=ZERO
    def add(j,v):
        # Round each summand before addition: endpoints remain dyadic and do
        # not accumulate the product of unrelated action-gap denominators.
        lower[j]+=floor_grid(v); upper[j]+=-floor_grid(-v)
    for u,(row,rhs) in zip(y,lp.eq):
        constant+=floor_grid(u*rhs)
        for j,c in row.items(): add(j,-u*c)
    for u,(row,rhs) in zip(lam,lp.ub):
        constant+=floor_grid(-u*rhs)
        for j,c in row.items(): add(j,u*c)
    return constant+sum((floor_grid(min(cl*l,cl*u,cu*l,cu*u)) for cl,cu,(l,u) in zip(lower,upper,lp.bounds)),ZERO)
...
def solve(d,mode='scaled',seconds=30):
    ...
    proof=dict(schema='nbo-r43-regret-v2',model=d,mode=mode,y=y,lambda_nonnegative=lam,policy=policy,lower=lower,upper=upper)
