...
old_rect=core.rectangular;old_build=core.build;original_repair=core.old.repair;Z=F(0)
...
def run(name,nodes=1,seconds=120):
    ...
    checked=verify_price(R/'proofs'/name/'restart.json.gz')
    incumbent=[[list(map(F,row)) for row in period] for period in inherited['policy']]
    incumbent_upper=F(checked['upper'])
    def repair_or_incumbent(data,proposal,reference):
        repaired=original_repair(data,proposal,reference)
        if incumbent_upper<repaired[1]:
            return incumbent,incumbent_upper,Z,F(checked['max_regret'])
        return repaired
    # Supplying an already verified incumbent cannot invalidate a lower bound.
    # If it replaces a repaired corner, it has no larger cost than that corner.
    core.old.repair=repair_or_incumbent
    started=time.perf_counter()
    try:obj=core.search(d,enhanced=False,seconds=seconds,nodes=nodes,target=F(1,1000),node_seconds=15)
    finally:core.old.repair=original_repair
...
if __name__=='__main__':
    ...
    for name in [s for s in sys.argv[1:] if s!='--full']:run(name,nodes=2047 if full else 1,seconds=120)
