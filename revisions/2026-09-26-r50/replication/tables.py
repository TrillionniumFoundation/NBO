...
ROOT=Path(__file__).resolve().parent.parent;REPO=ROOT.parents[1];GEN=ROOT/'paper/generated';GEN.mkdir(exist_ok=True)
...
def main():
 ...
 wr('diffuse_all.tex',[[v['case'],v['method'].replace('highs-',''),v['N'],num(v['lower']),num(v['upper']),sci(v['width']),f"{v['all_in_seconds']:.3f}",f"{v['proof_bytes']/1024:.1f}"] for v in d])
 ...
 wr('finite_all.tex',[[v['case'],v['method'],int(v['budget_seconds']),num(v['lower']),num(v['upper']),sci(v['width']),f"{v['all_in_seconds']:.2f}",v['nodes'],f"{v['peak_mib']:.1f}"] for v in finite])
 wr('price_audit.tex',[[v['case'],v['masks'],v['exact_occupation_certificates'],v['pruned_masks'],num(v['ideal_lower']),num(v['ideal_upper']),sci(F(v['ideal_upper'])-F(v['ideal_lower']))] for v in price])
 ...
 old=REPO/'revisions/2026-09-26-r48/results';ar=ROOT/'paper/archive_r48/generated';ar.mkdir(exist_ok=True)
 for category,names in [('primary',['warranty0','warranty1','warranty2','inventory0','inventory1','inventory2','queue0','queue1','queue2','ties0','ties1','ties2']),('fleet',['fleet0','fleet1'])]:
  ...
  for c in names:
   ...
   rows.append([label,num(u),sci(dw),sci(wid),f'{100*float(wid/u) if u else 0:.4f}',p['nodes'],str(p['stop']).replace('_',r'\_')])
