...
The main economic result is a strictly positive, verified lower bound on the value of access to preference adjustment. At the central initial state and adjustment-cost coefficient $k=2$, access raises optimal welfare by at least $.04094994$ in the model's fixed utility units. The corresponding lower bounds are $.06274788$ at $k=.5$ and $.00781212$ at $k=8$. These are comparisons with the \emph{optimal} no-adjustment economy, not merely with an arbitrarily chosen no-adjustment policy. A feasible learned policy supplies the flexible economy's lower bound. An auxiliary market-deflator witness supplies the restricted economy's upper bound while allowing every original adaptive consumption and portfolio policy. A scalar state used to construct the witness does not change the economic opportunity set.

The computational argument has three parts. First, smooth one-sided witnesses bound the original stopped problem without assuming a smooth optimal value or regularity of the boundary under every control. Second, fitting and certification are separate operations. A polynomial fitted to a continuous-generator equation is converted to outward Bernstein coefficients and refined until its residual enclosure reaches the requested level. Independently, Gaussian polynomial moments evaluate a trained time-control policy under the exact continuous stopping contract. Third, a bounded-source Feynman--Kac construction and exponential localization potentials turn the no-adjustment market-deflator relaxation into a verifiable economic upper bound. Weighted Taylor boxes refine its initial value enclosure from width $.00568$ to below $.000320$.

The distinction between evaluation accuracy and optimality accuracy matters. At $k=2$ the original-payoff learned policy has a value enclosure of width $1.33\times10^{-5}$, but its current regret upper bound is $.10634$, not $.0000133$. This replaces the previous $8.33956$ bound for a different retained policy with a substantially smaller original-model certificate; it does not establish the declared $.01$ full-policy target. The positive access-welfare result is valid nevertheless because it pairs the feasible flexible policy with a substantially tighter upper bound on the restricted optimum. The paper therefore establishes a specific economic inequality rather than interpreting an unresolved approximation as an optimal policy surface.
...
\end{equation}
\end{theorem}
The theorem concerns discounted adjustment expenditures, not a pointwise ordering of controls. A re-solved $V_u/k$ is not a comparative-static proof. The central full-value enclosures in Table~\ref{r9:tab:original} are still too wide for informative optimal-budget secants. The tightly enclosed expenditures in Table~\ref{r9:tab:economics} are those of the specified learned policies.
...
\input{revisions/2026-09-22-r9/paper/tables/restricted_dual}
...
\begin{table}[t]
\centering\scriptsize
\caption{Certified original-model expenditures and value of access}\label{r9:tab:economics}
\input{revisions/2026-09-22-r9/paper/tables/economics}
\par\smallskip\footnotesize The expenditure column evaluates the displayed learned policy. The access columns bound the difference between the two \emph{optimal} values. The lower endpoint pairs a feasible flexible policy with the restricted dual upper; the upper endpoint pairs the flexible upper with a feasible restricted policy. The positive lower endpoints are informative economic guarantees even though the two-sided access intervals are not narrow estimates of the welfare magnitude.
\end{table}

The welfare lower bounds remain strictly positive at all three prices. At $k=2$, the quadrature enclosure of the restricted witness is more than two orders of magnitude narrower than the certified access gain. This comparison establishes a minimum economically meaningful gain without assuming that the flexible learned policy is optimal. It does not identify the exact access gain, an optimal adjustment-budget magnitude, or an optimal pointwise policy ordering.
...
\input{revisions/2026-09-22-r9/paper/tables/actions}
...
\input{revisions/2026-09-22-r9/paper/tables/absolute}
...
\input{revisions/2026-09-22-r9/paper/tables/external}
...
\input{revisions/2026-09-22-r9/paper/tables/external_absolute}
...
\section{Conclusion}
The computations establish a positive minimum value of access to preference adjustment in the original continuous economy. They do so by combining a feasible learned policy with an independently certified upper bound on the restricted optimum, rather than by assuming either learned rule is optimal. The restricted economy is enclosed within $.00905$ at the central state. The flexible policy's evaluation is much more accurate than its present $.10634$ regret bound at $k=2$; the full $.01$ flexible-policy objective remains a distinct numerical target.

The methodological results give operational ways to improve and audit these calculations: fitted Bernstein witnesses, stopped Gaussian-moment evaluation, market-deflator localization, interval neural derivatives, and a non-enumerative continuous-action oracle. The coupled comparison problem also has a rigorous absolute lower bound and a frozen multi-budget, multi-dimensional holdout record. Together these results sharpen the connection between neural computation and economic conclusions while leaving every inherited derivation, experiment, and unresolved diagnostic available for further review.
