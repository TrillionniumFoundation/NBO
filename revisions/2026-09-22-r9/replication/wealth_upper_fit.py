...
def run(k=2,deg=(4,5,12),mesh=(13,31,31),iterations=25):
 ...
 m=np.prod(np.array(deg)+1);coef=np.load(OUT/f'wealth_shaped_coeff_k{k:g}.npy') if (OUT/f'wealth_shaped_coeff_k{k:g}.npy').exists() else np.full(m,-2.);Qs=[];bs=[];history=[]
 ...
 (OUT/f'wealth_upper_dense_k{k:g}.json').write_text(json.dumps(out,indent=2)+'\n')
 ...
 out['dense_residual_diagnostic']=worst;(OUT/f'wealth_upper_dense_k{k:g}.json').write_text(json.dumps(out,indent=2)+'\n')
