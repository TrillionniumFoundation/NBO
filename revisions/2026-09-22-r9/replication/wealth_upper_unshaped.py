...
def run(k=2,deg=(4,5,12),mesh=(9,10,24),iterations=25):
 ...
 for it in range(iterations):
  ...
  c,th,p=selector(M,coef,u,x,k);Q,b=constraint(M,u,x,c,th,p,k);res=Q@coef-b
  hist={'iteration':it,'candidate_upper_no_residual':float(objective@coef+.1*np.log(1.25)),'max_collocation_residual':float(res.max()),'cuts':sum(len(v) for v in bs),'seconds':time.perf_counter()-start};history.append(hist);print(hist,flush=True);np.save(OUT/f'wealth_coeff_k{k:g}.npy',coef)
 ...
 (OUT/f'wealth_upper_k{k:g}.json').write_text(json.dumps(out,indent=2)+'\n')
 ...
 out['dense_residual_diagnostic']=worst;(OUT/f'wealth_upper_k{k:g}.json').write_text(json.dumps(out,indent=2)+'\n')
