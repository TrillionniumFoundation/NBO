...
from certified_arithmetic import derive,verify_positive,rat,downward,upward
import copy

def identity(target,out):
    seal=json.loads((out/'expected.json').read_text())
    if seal['schema']!='nbo-r13-witness-v1':raise ValueError('unknown witness schema')
    check(target,seal['canonical_manifest_sha256'])
    allowed=set(seal['files'])|{'expected.json','independent_validation.json'}
    if {p.name for p in out.iterdir()}!=allowed-({'independent_validation.json'} if not (out/'independent_validation.json').exists() else set()):raise ValueError('unlisted or missing witness output')
...
def procurement(eng,out):
    src=json.loads((out/'procurement.json').read_text());opens=json.loads((out/'positive_witnesses.json').read_text());G=float(eng.b.terminal[eng.b.center]);h=.01;eta=1e-8;rows=[];maxgap=0.
        ...
        for adj,F,m in itertools.product((True,False),(0.,.2,.4,.6,.8,1.),range(1,9)):
            ...
            for shift in (-1,0,1):
                ...
                for si,s in enumerate(SIGNS):
                    ...
                    if s=='positive':
                        rec=opens[tag]
                        close(rec['closure_value'],first[s][0],'positive closure supremum')
                        gap=verify_positive(eng.fm,q,rec,.8,adj)
                        if gap+2*eng.arithmetic['bounds']['bellman_value']>=eta:raise ValueError('positive response regret exceeds tolerance')
                        close(rec['certified_regret_upper'],gap+2*eng.arithmetic['bounds']['bellman_value'],'positive regret account')
            for s in SIGNS:
                wm,w,wp=[f[s][0] for f in vals]
                # Independent exact-rational accounting, including the actual
                # binary64 probe spacings and the positive-part transfer.
                hm=rat(.42425)-rat(.42425-h);hp=rat(.42425+h)-rat(.42425)
                lo=max(rat(0),(rat(w)-rat(wm)-2*rat(EPS)-rat(eta))/hm)
                hi=min(rat(1),(rat(wp)-rat(w)+2*rat(EPS)+rat(eta))/hp)
                if lo>hi:raise ValueError('service interval empty')
                cap=rat(.02)*rat(F)**2
                rows.append(dict(id=key+'.'+s,adjustment=adj,fee=F,term=m,service_lower=downward(lo),service_upper=upward(hi),grant_executable=upward(max(rat(0),rat(G)-rat(w)+rat(EPS)+cap+rat(eta))),grant_lower=downward(max(rat(0),rat(G)-rat(w)-rat(EPS)+cap))))
    ...
    for b,kap,adj in itertools.product((.998,1.02),(.0198,.0202),(True,False)):
        ...
        lo=[rat(b)*rat(r['service_lower'])-rat(r['grant_executable'])-rat(kap)*(r['term']-1)/8 for r in rr];hi=[rat(b)*rat(r['service_upper'])-rat(r['grant_lower'])-rat(kap)*(r['term']-1)/8 for r in rr];best=max(range(len(rr)),key=lambda i:lo[i])
        gap=lo[best]-max([rat(0)]+[x for i,x in enumerate(hi) if i!=best])
    ...
    for want in src['choices']:
        ...
        else:choice=rr[best]['id'];gap=downward(lo[best]-max([rat(0)]+[v for i,v in enumerate(hi) if i!=best]))
    ...
    return dict(offers=len(rows),strict_positive_witnesses=len(opens),minimum_positive_share=min(r['positivity_margin'] for r in opens.values()),bellman_parameter_queries=96*3,price_box_corners=corners,max_value_discrepancy=maxgap)
...
def negative_tests(eng,arr,target,out):
    ...
    rejects('understated_arithmetic_allowance',lambda:derive(eng.b,eng,1e-14))
    badfm=copy.copy(eng.fm);badfm.rows=list(eng.fm.rows);badfm.rows[0]=eng.fm.rows[0].copy();badfm.rows[0].data[0]=2.
    from types import SimpleNamespace
    rejects('first_date_row_mass_independent',lambda:derive(eng.b,SimpleNamespace(fm=badfm),EPS))
    rec=copy.deepcopy(next(iter(json.loads((out/'positive_witnesses.json').read_text()).values())))
    rec['action'][2]=0.;rec['positivity_margin']=0.
    rejects('zero_positive_procurement_action',lambda:verify_positive(eng.fm,np.zeros(len(eng.fm.actions)),rec,.8,True))
    with tempfile.TemporaryDirectory() as td:
        unlisted=out/'UNLISTED_FIXTURE'
        try:
            unlisted.write_text('untrusted extra witness')
            rejects('unlisted_output_inventory',lambda:identity(target,out))
        finally:unlisted.unlink()
    return results
...
def main():
    ...
    eng.arithmetic=derive(b,j,EPS)
    claimed=json.loads((args.out/'arithmetic.json').read_text())
    if claimed!=eng.arithmetic:raise ValueError('arithmetic derivation receipt disagrees with independent recomputation')
    with np.load(args.out/'certificate_arrays.npz',allow_pickle=False) as z:arr={k:z[k] for k in z.files}
    result=dict(schema='nbo-r13-independent-check-v1',scope='Independently recoded binary64 witness reconstruction; exact-rational data-dependent roundoff bounds and directed scalar procurement arithmetic. Not a constructor/diffusion certificate or formal proof assistant.',input_manifest_sha256=digest(args.out/'expected.json'))
    result['arithmetic']=eng.arithmetic
